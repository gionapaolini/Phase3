...
public class PlanetMeshGen {
    ...
    public Mesh generateMesh (Settings setting) {
        planetRadius = setting.getRadius();
        
        Mesh mesh = new Mesh();
        
        heightmapWidth = setting.getHeightmapwidth();
        ...
        for( int i = 0; i < thetaSamples; i++ ) {
            ...
            for( int j = 0; j < gammaSamples; j++ ) {
                ...
                else if(setting.getTypeOfAlgorithm()==2){
                    SimplexNoise noise = new SimplexNoise();
                    
                    height = setting.getAmplitude()* (float) noise.noise(pt.x/setting.getX_scale(), pt.y/setting.getY_scale(), pt.z/setting.getZ_scale());
...
//                
        ...
        mesh.updateBound();
        //generateNavigationMesh(setting);
        return mesh;
    }
    
    private void generateNavigationMesh(Settings setting){
     ...
     * 
     */
    public void generateHeightmap(Settings setting) {
        // generateHeightmap( 600, 19380400, 50, 90, 25000, .70f, .3f );
        generateHeightmap( setting.getHeightmapwidth(),
                           setting.getSeed(),
                           setting.getNumIslands(),
                           setting.getIslandRadius(),
                           setting.getIterations(),
                           setting.getDisplacement(),
                           setting.getSmoothing());
    }
